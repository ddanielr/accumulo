...
import org.apache.accumulo.server.validation.system.CompactionFactoryValidator;
...
public interface SystemPluginValidator {
  ...
  /**
   * Validate a plugin type based on the current configuration. If something is not valid,
   * implementations should log any configuration issues at the warning level before finally
   * returning false.
   *
   * @param context - ServerContext used for validation
   */
  ...
  static List<SystemPluginValidator> validators() {
    return List.of(new CompactionFactoryValidator(), new SystemBalancerValidator(),
